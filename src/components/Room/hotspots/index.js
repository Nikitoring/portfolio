...
const Hotspots = (props) => {
  ...
  const listHintContent = hotspots.map(item => <HintContent key={item.id} type={item.type} id={item.id} title={item.content.title} body={item.content.body} isChecked={item.isChecked} />)
  return (
